...
def ppxf_sauron(spectrumfl, z_guess=0.004, sigma_guess=170., simple = False, wlspec=False):
    ...
    FWHM_tem = 4  # Vazdekis+10 spectra have a constant resolution FWHM of 2.51A.
    #FWHM_tem =  # Vazdekis+10 spectra have a constant resolution FWHM of 2.51A.
    ...
    modelwl = hdu[:,0]
    wh_wifis = np.where((modelwl >= 4800) & (modelwl <= 5300))[0]
    modelwl_wifis = modelwl[wh_wifis]
    wl_rebin = np.linspace(modelwl_wifis[0], modelwl_wifis[-1], num = len(modelwl_wifis))
    ssp_rebin = np.interp(wl_rebin, modelwl_wifis, ssp[wh_wifis])

    #h2 = hdu[0].header
    #lamRange2 = h2['CRVAL1'] + np.array([0., h2['CDELT1']*(h2['NAXIS1'] - 1)])
    lamRange2 = [wl_rebin[0], wl_rebin[-1]]
    sspNew, logLam2, velscale_temp = util.log_rebin(lamRange2, ssp_rebin, velscale=velscale/velscale_ratio)
    ...
    if simple:
        templates = np.empty((sspNew.size))
    else:
    ...
    sigma = FWHM_dif/2.355/(wl_rebin[1]-wl_rebin[0])  # Sigma difference in pixels

    if simple:
        x = pd.read_csv(conroydict[(fullage[2],fullZ[1])], delim_whitespace = True, header=None)
        hdu = np.array(x)
        ...
        templates[:] = sspNew/np.median(sspNew)  # Normalizes templates
    else:
        for j in range(len(fullage)):
            for k in range(len(fullZ)):
        #for j, file in enumerate(vazdekis):
                x = pd.read_csv(conroydict[(fullage[j],fullZ[k])], delim_whitespace = True, header=None)
                hdu = np.array(x)
                #hdu = np.loadtxt(conroydict[(fullage[j],fullZ[j])])
                ssp = hdu[:,73]
                ...
                ssp = ndimage.gaussian_filter1d(ssp_rebin, sigma)
                sspNew, logLam2, velscale_temp = util.log_rebin(lamRange2, ssp, velscale=velscale/velscale_ratio)
                templates[:, j, k] = sspNew/np.median(sspNew)  # Normalizes templates
    ...
    component = [0]*len(fullage) + [1]*len(fullZ)
    regul_err = 0.013

    if simple:
        pp = ppxf(templates, galaxy, noise, velscale, start,
                  goodpixels=goodPixels, plot=False, moments=2,\
                  degree=4, vsyst=dv, velscale_ratio=velscale_ratio, quiet=True)# component=component, \
                          #reg_dim=len(fullage)*len(fullZ))
    else:
        pp = ppxf(templates, galaxy, noise, velscale, start,
                  goodpixels=goodPixels, plot=True, moments=2,\
                  degree=4, vsyst=dv, velscale_ratio=velscale_ratio, regul=1./regul_err)# component=component, \
                          #reg_dim=len(fullage)*len(fullZ))
...
if __name__ == '__main__':
    ...
    pp = ppxf_example_kinematics_sauron()
